...
class ClinicalEncoder(nn.Module):
    ...
    Wraps pytorch_tabnet's TabNet to expose embedding output.

    Input:  (batch, n_features) numpy float32
    ...
    def __init__(
        ...
        self.tabnet = TabNet(
            input_dim=input_dim,
            output_dim=n_d,           # TabNet internal output, not final embedding
            n_d=n_d,
            n_a=n_a,
            n_steps=n_steps,
            gamma=1.3,
    ...
    def forward(self, x):
        # TabNet returns (output, M_loss) where M_loss is sparsity regularization
